...
/// HCL-新增多视频导出
- (void)getVideoOutputPathWithAsset:(PHAsset *)asset index:(NSInteger)index success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure {
    ...
    [[PHImageManager defaultManager] requestAVAssetForVideo:asset options:options resultHandler:^(AVAsset* avasset, AVAudioMix* audioMix, NSDictionary* info){
        ...
        if ([presets containsObject:presetName]) {
            ...
            NSString *outputPath = [NSHomeDirectory() stringByAppendingFormat:@"/tmp/video-%@-%ld.mp4", [formater stringFromDate:[NSDate date]],index];
